...
-- ║   Modelo Relacional v4.2 — Script DDL Oracle SQL                 ║
-- ╠══════════════════════════════════════════════════════════════════╣
...
-- │  v4.1  Módulo Visitas Frecuentes — 18 tablas:                     │
-- │    · FRECUENTES_RESIDENTE (nueva) — vincula residentes con sus    │
-- │      visitantes frecuentes; soporte soft-delete (activo=0)        │
-- │    · VISITAS.cantidad_personas (nueva columna) — headcount del    │
-- │      grupo para formulario rápido sin listar cada persona          │
-- │    · VEHICULOS_VISITA.descripcion_tipo (nueva columna) — detalle  │
-- │      obligatorio cuando tipo = 'OTRO' (moto eléctrica, etc.)      │
-- │    · TRG_AUTO_FRECUENTE — auto-popula FRECUENTES_RESIDENTE al     │
-- │      insertar en REGISTRO_VISITA; reactiva si estaba oculto       │
-- │    · TRG_FRECUENTES_UPD — actualizado_en automático               │
-- │    · SP_LIBERAR_VISITA_FRECUENTE — crea visita completa con QR    │
-- │      solo con datos variables del día (grupo, vehículo, tiempo QR)│
-- │    · VW_VISITANTES_FRECUENTES — panel del residente con stats de  │
-- │      visitas anteriores y último vehículo para pre-poblar el form │
-- └────────────────────────────────────────────────────────────────────┘
...
CREATE TABLE RESIDENTES (
    ...
    email               VARCHAR2(254 CHAR),             -- RFC 5321: máx 254 chars
...
CREATE TABLE CUOTAS_ARRIENDO (
    ...
    CONSTRAINT CHK_CUOTA_ESTADO
        CHECK (estado IN ('PENDIENTE','PAGADA','VENCIDA','EN_MORA'))
);
...
COMMENT ON COLUMN CUOTAS_ARRIENDO.estado       IS 'PENDIENTE | PAGADA | VENCIDA | EN_MORA';
...
CREATE TABLE BUZON (
    ...
    foto_captura        CLOB,                            -- Base64 foto tomada por el portero (CONFIRMAR_VISITA)
    empresa_mensajeria  VARCHAR2(100 CHAR),               -- Solo para PAQUETE
    numero_guia         VARCHAR2(50  CHAR),               -- Solo para PAQUETE
    ...
    CONSTRAINT CHK_BUZON_TIPO
        CHECK (tipo IN ('PAQUETE','AVISO','CONFIRMAR_VISITA')),
...
COMMENT ON COLUMN BUZON.tipo                   IS 'PAQUETE | AVISO | CONFIRMAR_VISITA';
...
COMMENT ON COLUMN BUZON.empresa_mensajeria     IS 'Empresa de mensajería o reparto. Solo para PAQUETE.';
COMMENT ON COLUMN BUZON.numero_guia            IS 'Número de guía del paquete. Solo para PAQUETE.';
...
-- ══════════════════════════════════════════════════════════════════
-- Secuencias  : 18 (SEC_TIPOS_DOCUMENTO, SEC_APARTAMENTOS, ..., SEC_REGISTROS_ACCESO)
-- Tablas      : 18
--   1.  TIPOS_DOCUMENTO
--   2.  APARTAMENTOS
--   3.  PARQUEADEROS
...
--   10. PAGOS
--   11. ALERTAS_PAGO
--   12. VISITAS              (+ cantidad_personas en v4.1)
--   13. QR_ACCESOS
--   14. VISITANTES           (nueva en v4.0 — reemplaza PERSONAS_VISITA)
--   15. VEHICULOS_VISITA     (+ descripcion_tipo en v4.1)
--   16. REGISTRO_VISITA      (nueva en v4.0 — reemplaza PERSONAS_VEHICULO)
--   17. FRECUENTES_RESIDENTE (nueva en v4.1 — módulo visitas frecuentes)
--   18. REGISTROS_ACCESO
-- Triggers    : 17
...
--   ~22 explícitos por FKs y búsquedas frecuentes
--   (IDX_FREC_RESIDENTE eliminado en v4.2 por redundancia con UQ_FREC_RES_VIS)
-- ══════════════════════════════════════════════════════════════════
-- FIN DEL SCRIPT — modelo_relacional_v4.sql  v4.1
-- ══════════════════════════════════════════════════════════════════
